...
export const Table_price = ({ columns, editData, deleteData, setOnLoad, onLoad, ...props}) => {
  ...
  return (
    ...
    <div className="table-controls">
      <Input lambdaClassInput={"data_search"} type="search" value={search} onChange={ e => searching(e.target.value)} placeholder="Buscar precio por nombre o estado" aria-label="Buscar precio" />
    </div>
            ...
            prices.data?.map( ( data ) => {
              ...
              if(editData && deleteData){
                  return (
                    <tr key={values[0]}>
                      <th>{values[0]}</th>
                      <td data-label="Precio">{values[1]}</td>
